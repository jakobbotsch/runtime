...
void SsaBuilder::AddPhiArgsToNewlyEnteredHandler(BasicBlock* predEnterBlock, BasicBlock* enterBlock, BasicBlock* handlerStart)
{
    ...
    for (MemoryKind memoryKind : allMemoryKinds())
    {
        ...
        if (handlerMemoryPhi != nullptr)
        {
            if ((memoryKind == GcHeap) && m_pCompiler->byrefStatesMatchGcHeapStates)
            {
                ...
                assert(memoryKind > ByrefExposed);
                assert(predEnterBlock->bbMemorySsaNumOut[memoryKind] == predEnterBlock->bbMemorySsaNumOut[ByrefExposed]);
            ...
            DBG_SSA_JITDUMP("  Added phi arg for %s u:%d from " FMT_BB " in " FMT_BB ".\n",
                memoryKindNames[memoryKind], predEnterBlock->bbMemorySsaNumOut[memoryKind], predEnterBlock->bbNum,
                handlerStart->bbNum);
        }
    }
}
