...
public:
    ...
    void Finish(Statement* stmt)
    {
        ...
        else
        {
            ...
            {
                // We need somewhere to store both the 'first' and 'last' nodes
                // of the (sparse) linked list, yet we cannot store it in the
                // top level node since it is part of the list and should have
                // null prev/next links.
                // This case is extremely rare so just allocate a new stand-in
                // node for it.
                assert(firstNode == lastNode);
                rootNode = m_compiler->gtNewNothingNode();
                rootNode->gtNext  = firstNode;
                firstNode->gtNext = nullptr;
            }
            else
            {
                assert((rootNode->gtPrev == nullptr) && (lastNode->gtNext == nullptr));
                assert(lastNode->OperIsLocal() || lastNode->OperIsLocalAddr());
            }

            firstNode->gtPrev = nullptr;
            rootNode->gtPrev  = lastNode;
        }

        stmt->SetTreeList(rootNode);
    }
