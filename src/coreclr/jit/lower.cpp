        ...
        case IAT_PVALUE:
        {
            ...
            if (hasIndirectionCell)
            {
                for (unsigned i = 0; i < call->fgArgInfo->ArgCount(); i++)
                {
                    ...
                    {
                        assert(entry->isTmp || entry->GetNode()->AsUnOp()->gtGetOp1()->OperIsLocal());
                        if (entry->isTmp)
                        {
                        ...
                        {
                            cellAddr = comp->gtClone(entry->GetNode()->AsUnOp()->gtGetOp1());
                        }
                        break;
                    }
                }
...
GenTree* Lowering::LowerVirtualStubCall(GenTreeCall* call)
{
    ...
    else
    {
        ...
        assert(indirectionCellArg->isTmp || indirectionCellArg->GetNode()->AsUnOp()->gtGetOp1()->OperIsLocal());
        GenTree* addr;
        if (indirectionCellArg->isTmp)
        {
            addr = comp->gtNewLclvNode(indirectionCellArg->tmpNum, TYP_I_IMPL);
        }
        else
        {
            addr = comp->gtClone(indirectionCellArg->GetNode()->AsUnOp()->gtGetOp1());
        }
